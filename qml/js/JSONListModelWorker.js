...
WorkerScript.onMessage = function(message) {
    if (message.clearModel)
        message.model.clear();

    if (message.json === "")
        return;

    var objectArray = message.query !== "" ? jsonPath(message.json, message.query) : message.json;

    if (typeof(message.sortFuntion) === "function" && objectArray)
        objectArray.sort(message.sortFuntion)

    for (var key in objectArray) {
        var jo = objectArray[key];

        if (message.attachedProperties)
            jo = addMissingProperties(jo, message.attachedProperties)
